    ...
    for event in events:
        ...
        if event_tokens[1] == 'NOTEON' and event_tokens[0] in ['P1', 'P2', 'TR']:
            ...
            if new_midi_note >= instag2min[instag] and new_midi_note <= instag2max[instag]:
                events_transposed.append(
                    '{}_NOTEON_{}'.format(instag, new_midi_note))
        else:
...
def tx1_paper_augment(
        ...
        instag2min=_INSTAG2MIN,
        instag2max=_INSTAG2MAX):
    if trim_padding:
    ...
    if augment_transpose:
        ...
        events = tx1_transpose(events, transpose_amt, instag2min, instag2max)
