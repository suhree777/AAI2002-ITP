...
window_length = sample_rate * 4 # Sliding window size per instrment
embedding_dim = 256
num_epoch = 4
batch_size = 256
...
for index, instrument_name in enumerate(instrument_list):
    ...
    model.summary()
    model.fit(X, y, epochs=num_epoch, batch_size=batch_size)

    mode_path = os.path.join(model_folder, f"{instrument_name}_model.keras")
    model.save(mode_path)
    print(f'Model {instrument_name}{sample_rate}_model.keras saved to {model_folder}')
