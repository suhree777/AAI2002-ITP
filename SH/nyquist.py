...
def analyze_frequency_content(midi_path):
    ...
    print(f"Highest frequency in the MIDI file: {highest_frequency} Hz")
...
# Analyze the frequency content of the MIDI file
highest_frequency = analyze_frequency_content(midi_path)

# Determine the Nyquist rate
nyquist_rate = 2 * highest_frequency
print(f"Recommended sampling rate based on Nyquist theorem: {nyquist_rate} Hz")

# Use the recommended sampling rate for resampling MIDI data
times, instrument_data = resample_midi_instruments(midi_path, sample_rate_hz=nyquist_rate)
