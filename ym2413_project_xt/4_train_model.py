...
class MusicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.sequences[idx], dtype=torch.long).to(device), torch.tensor(self.labels[idx], dtype=torch.long).to(device)

class LSTMModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, lstm_units, num_classes, dropout_rate):
        super(LSTMModel, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)
        self.lstm = nn.LSTM(embedding_dim, lstm_units, batch_first=True, num_layers=3, dropout=dropout_rate)  # Increase layers
...
def train_model(train_loader, model, criterion, optimizer, scheduler, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for sequences_batch, labels_batch in loop:
            optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    model_save_path = os.path.join(model_save_dir, 'trained_lstm_model.pth')
