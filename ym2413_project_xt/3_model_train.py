...
if __name__ == '__main__':
    ...
    model_dir = 'ym2413_project_xt/3_trained_models'
    os.makedirs(model_dir, exist_ok=True)

    # Callbacks for monitoring and improving training
    checkpoint = ModelCheckpoint(
        os.path.join(model_dir, 'model_epoch{epoch:02d}_valLoss{val_loss:.2f}.keras'),
        save_best_only=False,  # Saves the model at each epoch regardless of performance
        monitor='val_loss',
        mode='min',
        verbose=1
    )
