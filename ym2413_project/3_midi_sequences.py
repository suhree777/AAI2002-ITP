...
def extract_events(midi_file_path, emotion_label):
    ...
    along with the emotion label, formatted as tuples.
    """
    ...
    for instrument in midi_data.instruments:
        channel_name = instrument.name if instrument.name in nes_channels else 'NO'
    ...
    for channel, notes in nes_channels.items():
        ...
        for note in notes:
            ...
            if wait_time > 0:
                events.append((f"WT_{int(wait_time * 100)}", emotion_label))
            events.append((note[1], emotion_label))
            last_time = note[0]
    ...
    for root, dirs, files in os.walk(midi_dataset_dir):
        ...
        for midi_file in files:
            if midi_file.endswith(('.mid', '.midi')):
                ...
                base_name = os.path.basename(midi_file).split('_aug')[0] + '.mid'
                emotion_label = emotion_labels.get(base_name, 'unknown')
...
if __name__ == '__main__':
    midi_dataset_dir = 'ym2413_project/output_with_generation'
