...
timeBetweenNotes = (
    1 /
    tempo.number) * 60 * 1000  #calculates the seconds between quarter notes
...
if not tracksSel or tracksSel[0] == "all":
    ...
    tracksToPlay = []
    for track in tracksSel:
        tracksToPlay += [n for n in score.elements[int(track)].flat.notes]
...
for p in tracksToPlay:  #pulls notes from selected tracks
    if isinstance(p, note.Note):
        notes.append([
            p.name, p.octave, p.quarterLength
        ])  #functional, only one where the program actually works
...
print(
    "The list of octaves with the number of notes in each octave, in order as they first appear in the song:"
)
...
print("Valid keys:",
      ...
      sep="\n")
print(
    "The list of notes with the number of times the note appears, in order as they first appear in the song:"
)
songKeyListNotes = []
for n in range(0, len(songkeylist)):
    songKeyListNotes.append((str(songkeylist[n]), str(keyCount[n])))
songKeyListNotes.sort(key=lambda x: x[0])
for s in songKeyListNotes:
    print(s[0] + ":\t" + s[1])
print("There are " + str(len(songkeylist)) + " notes.")

#removes notes to fit into a specific major
print(
    "Enter the notes you would like removed. To remove multiple notes, enter them with a space between each note. To remove no notes, simply hit enter."
)
...
for i in range(0, len(octaveslist)):
    ...
    octaveCount.append(count)
print(
    "The new list of octaves with the number of notes in each octave, in order as they first appear in the song:"
)
for o in range(0, len(octaveslist)):
    print(str(octaveslist[o]) + ":\t" + str(octaveCount[o]))
...
while True:
    inp = input(
        "Enter an octave you want replaced followed by the octave you want to replace it with, leaving a space between the two numbers. If you don't want to replace anything, just hit enter:\n"
    )
    if not inp:
        break
...
realFNO = firstNoteO if firstNoteO in octaveslist else octaveReplaceMap[
    firstNoteO]
...
for i in range(0, len(notes)):
    ...
    if i < len(notes) - 1:  #change octaves
        ...
        note2 = notes[i + 1][1]
        octaveDiff = (
            note1 if note1 in octaveslist else octaveReplaceMap[note1]) - (
                note2 if note2 in octaveslist else octaveReplaceMap[note2]
            )  #deals with octaves jank
        if octaveDiff > 0:
...
sheet.write("[] = lower octave\n() = higher octave\ntempo = " +
            (tempo.text if tempo.text else "") + " " + str(tempo.number) +
            "\n")
for i in range(0, len(ntList)):
    sheet.write("a")
    sheet.write(ntMap[ntList[i]])
    sheet.write("b = " + str(ntList[i]) + " quarter notes long (" +
                str(ntList[i] * timeBetweenNotes) + " ms)\t\t" +
                str(noteTypes[ntList[i]]) + " note")
